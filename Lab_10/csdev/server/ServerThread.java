...
public class ServerThread extends Thread {
	...
	void Move( MessageMove msg) throws IOException, InterruptedException, JAXBException {
...
//					ServerMain.getBattle(msg.battle - 1,  bat.getOpponentNic(userNic)),  ServerMain.getLobby(msg.battle - 1).getStatus()) );
		...
		MessageMoveResult m2 = new MessageMoveResult( bat.getPH(), bat.getStr(this.userNic),    result , bat.getStatus()  );
		
		//os.writeObject( new MessageMoveResult( bat.getPH(), bat.getStr(this.userNic),    result , bat.getStatus()  ));
		//os.writeObject(m2);
		MessageXml.writeMsg(os, m2);
		
		Protocol.testMessage(m2);
	...
	void wait( MessageWait msg) throws IOException, InterruptedException, JAXBException {
		while(true) {
			try {
				this.sleep(1000000);
		...
		MessageWaitResult m2 = new MessageWaitResult( ServerMain.getLobby(msg.battle - 1).getMove(),    ServerMain.getLobby(msg.battle - 1).getPH(),
...
//					ServerMain.getLobby(msg.battle - 1).getLastRes()  ));
	...
	boolean connect( MessageConnect msg ) throws IOException, JAXBException {
		...
		if ( old == null )
		{
			...
			MessageConnectResult m2 = new MessageConnectResult();
			MessageXml.writeMsg(os, m2);
			Protocol.testMessage(m2);
			return true;
		} else {
...
//				"User " + old.userFullName + " already connected as " + userNic ));
	...
	void letter( MessageLetter msg ) throws IOException, JAXBException {
		...
		if ( user == null )
		{
			...
			MessageLetterResult m2 = new MessageLetterResult( );
			MessageXml.writeMsg(os, m2);
			Protocol.testMessage(m2);
...
//						        ));
			...
			MessageLetterResult m2 = new MessageLetterResult(  ServerMain.getLobby(b - 1).getPH(), 
					b,     ServerMain.getBattle(b - 1, this.userNic  ),  ServerMain.getLobby(b - 1).getStatus());
			MessageXml.writeMsg(os, m2);
			Protocol.testMessage(m2);
	...
	void letter( MessageCheckMail msg ) throws IOException, JAXBException {
		
		ServerThread user = ServerMain.getUser( msg.usrNic );
		if ( user == null )
		{
			//os.writeObject( new MessageCheckMailResult() );
			MessageCheckMailResult m2 = new MessageCheckMailResult();
			MessageXml.writeMsg(os, m2);
			Protocol.testMessage(m2);
		} else {
	...
	void user( MessageUser msg ) throws IOException, JAXBException {
		...
		if ( battles != null ) {
			...
			MessageUserResult m2 = new MessageUserResult( battles );
			MessageXml.writeMsg(os, m2);
			Protocol.testMessage(m2);
		}
		else {
			//os.writeObject( new MessageUserResult( "Unable to get battles list" ));
			MessageUserResult m2 = new MessageUserResult("Unable to get battles list");
			MessageXml.writeMsg(os, m2);
			Protocol.testMessage(m2);
		}
	}
...
//			os.writeObject( new MessageCheckMailResult( "Unable to get mail" ));		
		...
		if ( battles != null ) {
			//os.writeObject( new MessageUserResult( battles ));
			MessageCheckMailResult m2 = new MessageCheckMailResult( battles );
			MessageXml.writeMsg(os, m2);
			Protocol.testMessage(m2);
		}
		else {
			//os.writeObject( new MessageUserResult( "Unable to get battles list" ));
			MessageCheckMailResult m2 = new MessageCheckMailResult("Unable to get battles list");
			MessageXml.writeMsg(os, m2);
			Protocol.testMessage(m2);
		}
	}
