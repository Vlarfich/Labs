...
public class Airlines implements Serializable {
	...
	public Airlines(String dep, String des, String n, String t, String time, String day) {
		Integer numb = Integer.parseInt(n);

		assert (validDep(dep)) : "Invalid departure";
		assert (validDep(des)) : "Invalid destination";
		assert (validNum(numb)) : "Invalid number";
		assert (validTime(time)) : "Invalid time";
	...
	public static Airlines read(Scanner fin, PrintStream out) throws IOException {
		...
		if (!nextRead(P_Destination, fin, out))
			return null;
		air.Destination = fin.nextLine();

		if (!nextRead(P_Type, fin, out))
			return null;
		air.Type = fin.nextLine();

		if (!nextRead(P_time, fin, out))
			return null;
		...
		if (!nextRead(P_days, fin, out))
			return null;
		air.days = fin.nextLine();

		if (!nextRead(P_num, fin, out))
			return null;
		str = fin.nextLine();
	...
	public String toString() {
		return // new String(
		num + "    \t\tDeparture: " + Departure + "    \t\tDestination: " + Destination + "    \t\tType: " + Type + "    \t\tTime: "
				+ time + "    \t\tDays: " + days //+ "\t\t" + "Num: " + num
		// )
		;
	}
